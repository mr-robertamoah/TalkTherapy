...
| TT-7.3b-b0 (SCRUM-232) | Shared pre-work: extract a reusable "resolve which org currently covers this counsellor/member/therapy" action (currently duplicated inline in `EnsureOrganizationCanPayForModelAction`); build a shared "compute a counsellor's compensation-driven share for a session" action (FIXED / PERCENTAGE-counsellorRate / PERCENTAGE-negotiatedRate / FREE); add a new field to `organization_counsellor_compensations` to actually store the negotiated rate when `basis: negotiatedRate` is chosen (previously a dead-end option — validated but with nowhere to store the value). Consumed by TT-7.3b-b, -d, and later -j. | Story | Low | 3 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-6.4b (done) |
...
| TT-7.3b-f1 (SCRUM-237) | Retainer client-access bug fix: a new early-exit at `EnsureUserHasAccessToTherapyAction`'s strict-gate call site — a retainer-covered engagement (org in good standing) grants access unconditionally, no `Transaction`/`PaymentAccessGrant` required, and the frontend stops rendering a Pay control for retainer-covered content. **Fully fixes the permanent-lockout bug.** Ship as soon as possible — no dependency on any other TT-7.3b sub-ticket. | Story | Medium | 3 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | — |
...
| TT-7.3b-k (SCRUM-242) | Client-facing disclosure UI: a modest, non-financial line ("This session is covered under [Organization]'s plan — no payment needed from you.") on both Pay-control surfaces (`TherapyPaymentDetails.vue`, `UnifiedTherapy.vue`), mirroring TT-7.4b/c's dual-surface convention. Never fee amounts, compensation percentages, or payout figures. No dependency on any new infra — can ship anytime. | Story | Low | 3 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-6.3b, TT-7.3a (both done) |
