...
| Key | Story | Type | Priority | Points | Source | Depends on |
|---|---|---|---|---|---|---|
| TT-3.1 | 1:1 video/audio session — scoped via `/start-feature` (SCRUM-7/SCRUM-26 review, 2026-09-11). Went through product-owner/project-manager/architect review plus two rounds of live web research on WebRTC infra vendors, since this is genuinely new infrastructure (no prior media/WebRTC precedent in this codebase, unlike almost every other epic so far which widened existing patterns). **Infra decision**: dual video-provider architecture behind one `VideoProviderInterface` — `DailyVideoProvider` and `ChimeVideoProvider` (AWS Chime SDK) adapters, active provider chosen via config per deployment (not a live runtime switch) — user's own explicit choice after cost/compliance research showed the originally-recommended LiveKit Cloud's HIPAA BAA only ships on its $500/mo Scale tier, while Daily.co and Amazon Chime SDK both offer a self-serve BAA at a small fraction of that cost at this platform's likely launch scale. Split into: **a** dual-provider backend (`VideoProviderInterface` + both adapters, config-driven selection, new N-participant-ready `VideoSession`/`video_session_participants` data model per architect recommendation — deliberately not hardcoded to 2 participants, so TT-3.2 only relaxes a business-rule constant rather than needing new schema — plus the low-frequency "call started/ended" app-level notification on the existing per-session `sessions.{id}` `PrivateChannel`, not the busier per-therapy/group `PresenceChannel`; actual WebRTC media signaling is handled by each provider's own SDK/servers, not Reverb) — ~13-18pts; **b** join-video payment-gate integration, reusing `EnsureStrictPaymentGateSatisfiedAction`/`EnsureUserCanAccessTherapyContentAction` exactly as messaging already does, so a client can't bypass a paid session's gate via video — 3pts; **c** frontend video client: a Vue composable wrapping whichever SDK is active (daily-js / Chime SDK JS) behind one common event surface, permission prompts, connection-quality indicator, join/leave UI, concurrent-with-chat layout — genuinely new UI category for this codebase, no "live media" precedent to extend — ~18pts; **d** disconnect/reconnect state handling: a transient media disconnect must NOT mutate `Session.status` (only the existing end-session action does), across both providers' own disconnect events — 5pts; **e** guardian/minor video-consent enforcement — user decided (2026-09-11) this needs an explicit new consent flow, not a silent extension of `Guardianship`'s existing text-content access model. An urgent interim fail-closed block shipped ahead of the real flow (PR #216, merged) once it was found that TT-3.1a-d had merged with NO age check at all, letting a minor with a guardian on file join video like an adult; `EnsureVideoIsAvailableForSessionAction` now hard-blocks any non-counsellor joiner who isn't an adult, no consent mechanism, until this sub-ticket's real flow replaces it. TT-3.1e's own full scope went through a dedicated `/start-feature` re-engagement (2026-09-11) — product-owner scoping surfaced 5 policy questions the user answered directly (notification cadence, consent-mode options, revocation immediacy, guardianship-deletion lapse, multi-guardian consent+visibility), then project-manager split it into 7 sub-tickets: **e-a** data model (per-therapy `video_consent_mode` setting PER_THERAPY/PER_SESSION + a `video_consents` audit table, polymorphic to Therapy or Session) — 5pts; **e-b** grant flow (any-one-guardian-suffices approval action, consent-mode-set action settable by counsellor or guardian, co-guardian audit-trail read) — 8pts; **e-c** invalidation flow (revoke + `DeleteGuardianshipAction`-triggered cascade through one shared path, reusing `EndVideoSessionAction` for immediate live-call termination — flagged highest-risk, safety-critical, same review depth as a money-movement ticket) — 8pts; **e-d** enforcement (replaces the interim block with the real per-mode check, must consult e-c's revoked state not just e-b's granted state) — 5pts; **e-e** guardian reminders (day-before + hour-before, only while consent outstanding, reuses existing scheduler patterns — no new infra) — 8pts, independent of e-c/e-d once e-b lands; **e-f** frontend (consent-mode toggle, guardian approve/revoke controls, audit-trail display) — 8pts; **e-g** regression/QA closeout, decision-log, feature doc, retires the interim-block docblock — 5pts. Sequencing: e-a → e-b → {e-c, e-e in parallel} → e-d (needs e-c done, not just started) → e-f → e-g. Explicitly out of scope, per the user's own direction: "a minor with no guardian at all can't keep the whole therapy" is a separate, broader ticket, not folded into e-a..g. ~47pts for e (floor not ceiling); **f** regression/QA closeout across both providers, decision-log entries, feature doc — 5pts. Sequencing: a → b/c/d (b and c/d can proceed in parallel once a's interface is settled) → e (e-a..e-g per above) → f. Public/non-participant visibility unaffected. | Story | Critical | ~44-49 base + ~47 (e-a..e-g); floor not ceiling per this project's own pattern | New; re-scoped via /start-feature (SCRUM-7 review 2026-09-11; TT-3.1e re-scoped via its own /start-feature 2026-09-11) | TT-1.2, TT-2.1 (both done) |
| TT-3.2 | Group therapy video rooms — scoped via `/start-feature` (SCRUM-27 review, 2026-09-14), now that TT-3.1 (all of it, including TT-3.1e-a..g) is fully merged. **User's own explicit scope-narrowing decision**: v1 is deliberately NOT "every group member joins video" — it's a counsellor-team room, with the group's own creator (the client who set it up) able to optionally join too. Ordinary members get no video access in this version at all. If the creator is a minor, they're hard-blocked from joining (interim fail-closed, mirroring TT-3.1's own original 1:1 precedent — no consent flow yet, that's a separate follow-up). In-call moderation (removing a disruptive/distressed participant) IS in scope, since neither provider adapter has any removal capability today. Every active counsellor gets equal host/owner video credentials — no "lead counsellor" concept, consistent with this codebase's existing precedent (TT-7.5b-b0, TT-2.6) of treating every active counsellor as an equal peer. Project-manager's own research found 3 of the assumed "new logic" pieces (creator-detection via `GroupTherapy::isUser()`, minor-detection via `clientIsMinor()`, anonymity-masking via `displayNameFor()`) already exist and generalize correctly with zero code changes — this ticket is mostly new authorization/moderation surface plus regression tests proving the above, not a ground-up build. Split into: **a** `GroupTherapy` authorization branch in `EnsureVideoIsAvailableForSessionAction` (counsellor-or-creator-only, minor-creator hard-block) + a type-aware participant-cap fix (`DailyVideoProvider`'s hardcoded `max_participants: 2` must be computed per session type, not flatly bumped — architect finding) — 5pts; **b** in-call participant removal, backend: a research spike on each provider's real ejection API shape (mirrors the TT-7.6a Paystack-spike precedent), then a 4th `VideoProviderInterface` method implemented on both adapters, plus a new counsellor-only `RemoveParticipantFromVideoSessionAction` and a new dedicated broadcast event — 8pts; **c** in-call participant removal, frontend: extends the existing unified `VideoCallPanel.vue`/`TherapyComponent.vue` surface (already shared by both individual and group chat — no new component), plus the removed client's own force-disconnect handling (distinct from TT-3.1d's transient-disconnect handling) — 5pts; **d** restricts `EndVideoSessionAction`'s "end for everyone" to counsellors-only specifically for GroupTherapy sessions (1:1 stays symmetric/unchanged) — 2pts; **e** regression/QA closeout across both providers, decision-log entries, feature doc (`scrum-27-group-therapy-video.md`), Playwright golden-path QA — 3pts. Sequencing: a → {b, d in parallel} → c → e. **Explicitly deferred to their own separate, later, unscoped follow-up tickets** (filed, not started): group-scoped guardian video-consent (SCRUM-313, mirrors how TT-3.1e was split out of TT-3.1), and full-membership group video — every `GroupTherapyMembership` member, not just counsellors+creator (SCRUM-314). | Story | High | 23 (up from a 13pt placeholder; 3 sub-items turned out to already exist, partially offsetting moderation being genuinely new) | Re-scoped via /start-feature (SCRUM-27 review 2026-09-14) | TT-3.1 (done) |
| TT-3.2-follow-up-1 | Group-scoped guardian video-consent (SCRUM-313) — placeholder, not yet scoped. Follow-up to TT-3.2, replacing its interim minor-creator block with a real consent flow, mirroring TT-3.1e's own split from TT-3.1. | Story | — | — | New (placeholder) | TT-3.2 |
| TT-3.2f | Full-membership group video (SCRUM-314) — scoped via `/start-feature` (2026-09-14), now that TT-3.2 (a-e) is fully merged. **User's own specific product shape** (not the placeholder's original "every member gets full two-way access" framing): a moderated "town hall" — ordinary `GroupTherapyMembership` members join **receive-only** (mic/camera never published, server-enforced via the provider's own primitive — Daily meeting-token `permissions.canSend`, Chime attendee `Capabilities` — never client trust), can **raise a hand**, and any active counsellor (no single "host," consistent with TT-3.2b/d's own "every counsellor is a peer" precedent) can **grant/revoke real, server-enforced speaking permission** from a live queue. Minor members join receive-only unconditionally; being GRANTED speaking permission is hard-blocked until SCRUM-313 ships (own interim block, does not wait on it, mirrors TT-3.2a's identical pattern) — closes a real detection gap found during scoping by adding a new per-membership minor snapshot (`group_therapy_user.was_minor_at_join`), since only a group's own creator had one before this. A granted member's own anonymity (reusing `GroupTherapy::isAnonymousFor()` as-is) determines audio-only-forever (anonymous) vs. full audio+video with a one-time pre-unmute reveal warning (non-anonymous). Participant/membership cap unified to **25** (both `ConstantsEnum::groupTherapyVideoMaxParticipants` and the group's own membership ceiling, applied immediately, no grandfathering — nothing is in production yet), plus a new app-level enforced cap for Chime specifically (it has no native room-size limit, unlike Daily). Revocation: any active counsellor can revoke any grant (peer model) manually, or automatically via an idle-expiry sweep (client-activity ping + scheduled sweep, ~5min default, config-driven — architect-designed, mirrors the existing `routes/console.php` scheduled-reminder idiom). No hardcoded simultaneous-speaker limit; hand-raise requires already being admitted; receive-only join reuses the existing `EnsureUserCanAccessTherapyContentAction` payment gate as-is. Architect's own schema design: a new append-only `video_session_speaking_grants` table (modeled on `video_consents`, anchored to `video_session_participants.id`) for grant/revoke audit history, and a separate small mutable `video_session_hand_raises` table (deliberately NOT columns on `video_session_participants`, whose own migration comment already rejects mutable current-state fields). Split into: **a** participant cap unification (10/50→25) — 3pts; **b** data model (both new tables + minor snapshot column) — 5pts; **c** research spike on each provider's real API shape for updating an ALREADY-CONNECTED participant's capabilities without a reconnect (mirrors the TT-3.2b ejection-API spike precedent) — 3pts; **d** receive-only join (+ Chime app-level cap, enforced inside `JoinVideoSessionAction`'s existing lock to avoid a TOCTOU race) — 8pts; **e** new required `VideoProviderInterface::updateParticipantCapabilities()` method on both adapters, sized as a floor not ceiling pending **c**'s findings — 8pts; **f** raise-hand + grant/revoke backend, with a single shared `RevokeSpeakingPermissionAction` reused by manual revoke/idle-sweep/removal-cleanup — 8pts; **g** minor-grant hard-block + anonymity-aware speaking mode — 5pts; **h** idle auto-expiry sweep — 8pts; **i** removal interop (clears hand-raise/granted state) — 2pts; **j** frontend, extending the existing unified `VideoCallPanel.vue`/`TherapyComponent.vue` surface (no new component) — 8pts; **k** regression/QA closeout + feature doc (`scrum-314-full-membership-group-video.md`) — 5pts. Sequencing: {a, b, c} → {d, e (e needs c)} → f → {g, h, i} → j → k. ~55-58pts (floor, not ceiling — two genuine research spikes and a new client-reported idle mechanism with no prior precedent in this codebase). **Explicitly deferred**: SCRUM-329, a placeholder to replace g's interim minor-grant block once SCRUM-313 (group-scoped guardian consent, still unstarted) ships. | Story | High | ~55-58 (floor, not ceiling) | Re-scoped via /start-feature (SCRUM-314 review 2026-09-14) | TT-3.2 (done) |
...
| TT-7.3b | **Scope corrected and split (2026-09-03, SCRUM-230 product-owner/project-manager/architect review)**. Originally framed as "split money that already came in" — investigation revealed no mechanism anywhere bills or collects money from an organization (TT-7.3a only built attribution/tracking); two live bugs found (org-paid counsellors currently earn $0; a retainer-covered therapy with strict payment gating permanently locks the client out). Full scope now: real org-side payment collection (both billing modes), compensation-driven payout split reusing TT-7.6's ledger, an org-admin reconciliation view, client-facing disclosure, and a refund-reconciliation hook for TT-7.7. Split into 12 sub-tickets below (~74 points total, comparable in scale to TT-7.6's own 47-point/5-way split). Full decision trail in `documentation/decision-log.md`'s 2026-09-03 SCRUM-230 entries. | Epic | — | ~74 (see a–k below) | New split of existing TT-7.3 (SCRUM-111); rescoped via SCRUM-230 review | TT-7.3a, TT-7.6 (both done) |
| TT-7.3b-a (SCRUM-231) | ✅ **Implemented** (PR #167, merged). Backend-only, no controller/route/UI (deferred to TT-7.3b-i). Paystack spike confirmed `chargeAuthorization` reuses an authorization indefinitely for both one-off and recurring charges, same currency-corridor constraint the platform already lives with. New `organization_payment_instruments` table (masked-display-only, mirrors `counsellor_payout_accounts`) + registration flow: a small nominal verification charge (Paystack has no free "verify this card" call) tracked as `pending_credit_amount`, credited toward the org's first real invoice once TT-7.3b-e exists. | Story | Medium | 8 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-6.1 (org verification, done) |
| TT-7.3b-b0 (SCRUM-232) | ✅ **Implemented** (PR #163, merged). New `negotiated_rate_amount` field wired through the full propose→counter-offer→accept negotiation flow, closing the previously dead-end `negotiatedRate` basis option. Two new shared actions for later sub-tickets: `GetActiveOrganizationCounsellorAction` (Counsellor+Organization → active affiliation) and `ComputeCounsellorCompensationShareAction` (FIXED / PERCENTAGE-counsellorRate / PERCENTAGE-negotiatedRate / FREE, with a defensive overflow guard on the percentage multiplication — security-engineer finding). | Story | Low | 3 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-6.4b (done) |
| TT-7.3b-b (SCRUM-233) | ✅ **Implemented** (PR #169, merged). `ChargeOrganizationForModelAction` computes the actual cost of a single Therapy/Session engagement — platform fee on the counsellor's own listed rate (architect-verified basis) + counsellor's compensation-driven share via TT-7.3b-b0 — and charges it to the org's saved payment instrument (TT-7.3b-a) via `PaystackClient::chargeAuthorization()`, producing a `Transaction` with `organization_id` set. Reused by TT-7.3b-c for pay-per-use; TT-7.3b-e (retainer, aggregated invoice periods) will need its own decision on how much of this to reuse (see decision log) — not assumed to already fit. | Story | Medium | 6 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-a, TT-7.3b-b0 |
| TT-7.3b-c (SCRUM-234) | ✅ **Implemented** (PR #171, merged). `TransactionService::initiateCharge()` routes a pay-per-use org-financed Therapy/Session charge through TT-7.3b-b's primitive (org's instrument, actual cost) instead of the member's own card at listed price — the collection-side half of the "org-paid counsellors earn $0" fix (the other half is TT-7.3b-d). GroupTherapy deliberately still uses the old member's-card path (TT-7.3b-b's own scope boundary). Real, deliberate behavior change to already-shipped TT-7.3a: an org charge with no instrument/compensation terms now fails outright rather than silently charging the member. Gained a `Cache::lock` closing a duplicate-charge race (security-engineer finding) — follow-up SCRUM-243 filed for a pre-existing charge-then-record ordering gap. | Story | Medium | 5 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-b |
| TT-7.3b-d (SCRUM-235) | ✅ **Implemented** (PR #173, merged). **Fully fixes the "$0 for org-paid counsellors" bug** — `GenerateCounsellorEarningsAction`'s org no-op is replaced (individual Therapy/Session only; GroupTherapy remains a deliberate no-op) with a real split re-deriving the counsellor's share via TT-7.3b-b's own shared primitives, fee taken as the remainder of `transaction.amount` so `net_amount + fee_amount == transaction.amount` holds by construction. A compensation/settings drift between charge-time and generation is flagged via a warning log (security-engineer finding) rather than silently absorbed as fee revenue. | Story | Medium | 8 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-b0, TT-7.6b (done) |
| TT-7.3b-e (SCRUM-236) | ✅ **Implemented** (PR #175, merged). Retainer billing/invoicing mechanism: new `organization_invoices` + `organization_invoice_lines` tables (one `Transaction` per settled invoice period, `Transaction::for()` polymorphically pointed at `OrganizationInvoice` — not a synthetic per-session `Transaction`); a new hook in `App\Actions\Session\ChangeSessionStatusAction`'s transition to `held` writes an invoice line per retainer-covered session; periodic aggregation/settlement job auto-charges the org's saved instrument via TT-7.3b-b, feeding TT-7.3b-d's earnings generation. Largest ticket — genuinely new recurring-billing domain, previously-nonexistent Session→billing coupling. | Story | High | 15 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-a, TT-7.3b-b, TT-7.3b-b0, TT-7.3b-d |
| TT-7.3b-f1 (SCRUM-237) | ✅ **Implemented** (PR #162, merged). Early-exit at `EnsureStrictPaymentGateSatisfiedAction`'s gate — a retainer-covered engagement (org in good standing) grants access unconditionally, no `Transaction`/`PaymentAccessGrant` required. **Fully fixes the permanent-lockout bug.** Scope note: the "stop rendering a Pay control for retainer-covered content" frontend piece was deferred to TT-7.3b-k — no client-facing "pay via org" UI exists yet for it to suppress. | Story | Medium | 3 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | — |
| TT-7.3b-f2 (SCRUM-238) | ✅ **Implemented** (PR #177, merged). Org-suspension enforcement: a `billing_suspended_at` (+ reason) column on `Organization` (mirrors `verified_at`'s single-current-state-flag precedent), single-writer from the invoice-settlement-failure path; enforced at the same TT-7.3b-f1 call site — a suspended org's retainer-covered members are blocked entirely (no personal-pay fallback) until the org settles. | Story | Medium | 5 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-e, TT-7.3b-f1 |
| TT-7.3b-g (SCRUM-239) | ✅ **Implemented** (PR #181, merged). `ReconcileOrgFinancedRefundAction`: callable-only mechanism (org admin notification + `CounsellorEarning` reversal, new terminal `reversed` status) for TT-7.7d to invoke later on an org-financed refund. No live UI trigger until TT-7.7 exists — independently testable via direct action invocation. Generic across both org-billing modes. Only `pending` earnings are auto-reversed; `processing`/`paid_out` earnings are flagged for manual review instead, since `RecordCounsellorPayoutStatusAction`'s own blanket status update could otherwise silently clobber a reversal (reviewer + security-engineer finding). This is what unblocks TT-7.7 per the existing resequencing. | Story | Medium | 5 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-d |
| TT-7.3b-i (SCRUM-240) | ✅ **Implemented** (PR #184, merged). Org payment-instrument onboarding UI: new `OrganizationPaymentInstrumentController` (admin-gated page + a Paystack-checkout-redirect `initiate` endpoint, mirroring `TransactionController::initiate()`'s JSON shape rather than the bank/momo destination flow's synchronous one), new `Organization/PaymentInstrument.vue` page linked from the org dashboard. `TransactionController::redirectUrlFor()`'s existing Organization-subject branch now lands the post-checkout callback here instead of the dashboard. Closes out the TT-7.3b epic. | Story | Low | 5 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-a |
| TT-7.3b-j (SCRUM-241) | ✅ **Implemented** (PR #179, merged). Org-admin reconciliation view (backend + frontend): read-only list of financed sessions, computed splits, transaction/invoice/payout/suspension status — reuses TT-6.6a's org-scoped-list convention and TT-6.3b's session-metadata-only (never clinical) visibility scoping. | Story | Medium | 8 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-c, TT-7.3b-d, TT-7.3b-e, TT-7.3b-f2 |
| TT-7.3b-k (SCRUM-242) | ✅ **Implemented** (PR #165, merged). Client-facing disclosure UI: a modest, non-financial line ("...covered under [Organization]'s plan with TalkTherapy -- no payment needed from you.") on both Pay-control surfaces (`TherapyPaymentDetails.vue`, `UnifiedTherapy.vue`), replacing the Pay control for retainer-covered clients only (pay-per-use org members are unaffected). Never fee amounts, compensation percentages, or payout figures. Security-engineer review caught and fixed an anonymity leak in an early draft — see decision log. | Story | Low | 3 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-6.3b, TT-7.3a (both done) |
...
| TT-7.4d-a (SCRUM-258) | ✅ **Implemented** (PR #199, merged). Group-therapy per-member payment, part 1 of 5 (SCRUM-256 product-owner/project-manager/architect review, 2026-09-10). Viewer-scoped payment status backend foundation: `TherapyTrait::latestTransactionFor(User $user)`, a single shared per-viewer query promoted from `SessionResource`'s existing inline `viewerTransaction` closure (TT-7.7b/e precedent) rather than a new pattern — closes a live leak where the unscoped `latestTransaction` surfaces one GroupTherapy member's pending/failed attempt to a different member. Prerequisite for b/c/d. No schema change needed — `Transaction.user_id` and `payment_access_grants`' `(user_id, for_type, for_id)` key already support per-member payment; the only actual live gap (`EnsureCanInitiateChargeAction`'s unscoped "already paid" guard) was found and fixed separately, first, in isolation (SCRUM-257, PR #198, in review). | Story | High | 3 | New split of TT-7.4d (SCRUM-256 review, 2026-09-10) | TT-7.4a, SCRUM-257 |
| TT-7.4d-b (SCRUM-259) | ✅ **Implemented** (PR #200, merged). Part 2 of 5. Client Pay UI enablement: lifts `usePayment.js`'s hardcoded `therapyType !== 'group'` exclusion using -a's viewer-scoped field, real Pay control on `TherapyPaymentDetails.vue`/`UnifiedTherapy.vue` for a group member's own flat per-head share (pricing decision: flat per-head, not a total-group-split — 2026-09-10). Mirrors TT-7.4b's own dual-surface pattern. | Story | High | 5 | New split of TT-7.4d (SCRUM-256 review, 2026-09-10) | TT-7.4d-a |
| TT-7.4d-c (SCRUM-260) | ✅ **Implemented** (PR #201, merged). Part 3 of 5. Client refund-request UI lift, same two Vue files, sequenced after -b to avoid overlapping diffs. No backend change expected — TT-7.7's refund epic is already transaction/user-scoped, not model-scoped. | Story | Medium | 3 | New split of TT-7.4d (SCRUM-256 review, 2026-09-10) | TT-7.4d-a |
| TT-7.4d-d (SCRUM-261) | ✅ **Implemented** (PR #202, merged). Part 4 of 5. Counsellor-facing per-member payment roster (`GroupTherapy::users()` + each member's scoped status via -a, single eager-loaded query — architect: MUST follow the `GetCounsellorCalendarSessionsAction`/TT-7.7e nested-eager-load precedent, not a per-member loop). Ships informational only, no blocking/readiness enforcement (that's TT-7.5b), but field-named so a later readiness feature can reuse it. **Anonymity decision (2026-09-10, user sign-off)**: this roster pierces the platform's otherwise-universal anonymity rule (`addedByUserIsMaskedFor`/`RequestResource`/`MessageResource` all currently mask identity from the counsellor too) — scoped narrowly to the payment-status field only. | Story | Medium | 8 | New split of TT-7.4d (SCRUM-256 review, 2026-09-10) | TT-7.4d-a (independent of b/c) |
| TT-7.4d-e (SCRUM-262) | ✅ **Implemented**. Part 5 of 5. Regression closeout: locked in `GetPayableAmountAction` already does flat-per-head correctly as-is (`tests/Unit/GetPayableAmountActionTest.php`); added an HTTP-level regression test proving two different members of the same group can each initiate a charge via the real route (`tests/Feature/GroupTherapyPaymentEpicRegressionTest.php`) — the one gap the epic's otherwise-thorough Action/Service-level tests didn't cover; decision-log entry for the anonymity exception. Closes the TT-7.4d epic — see `documentation/features/scrum-256-group-therapy-per-member-payment.md`. | Story | Low | 3 | New split of TT-7.4d (SCRUM-256 review, 2026-09-10) | TT-7.4d-a, -b, -c, -d |
...
| TT-7.5b | GroupTherapy payment-gated access — unblocked once TT-7.4d shipped in full (2026-09-10). Split (product-owner/project-manager/architect review, SCRUM-216, 2026-09-11) into: **b0** a new multi-counsellor toggle-authorization action (GroupTherapy supports several concurrently-ACTIVE counsellors, unlike TT-7.5a's single `counsellor_id` — a genuine new fork, not a mirror; any ACTIVE counsellor may toggle independently, matching the existing `activeCounsellors()` convention used for earnings split/org-payer eligibility) — 5pts; **b1** `strictPaymentGate` + a new sibling setting `allowFreeHistoricalAccess` (boolean, default `true`) on GroupTherapy create/update, initial value gated on whether an active counsellor already exists at creation time (not on `addedby`'s identity, since `createGroupTherapy` already accepts `counsellorIds` up front) — 3pts; **b2** page-load enforcement: widen `EnsureUserHasAccessToTherapyAction`/`EnsureStrictPaymentGateSatisfiedAction` to GroupTherapy, landing TWO non-optional widening fixes atomically (architect finding, not follow-up work): move `Therapy::getStrictPaymentGateAttribute()` into `TherapyTrait` (widening the type-hint without this is a SILENT bypass — the gate would simply never trigger for any GroupTherapy) and widen `GetRetainerCoveringOrganizationAction` off its `$therapy->counsellor` singular-relation assumption (a hard `TypeError` on GroupTherapy otherwise) — 8pts; **b3** session/chat enforcement for PER_SESSION, PLUS the new late-joiner temporal logic: when `allowFreeHistoricalAccess` is true, content dated before a member's own `group_therapy_user.created_at` (join date) stays visible regardless of payment, content dated after is gated — genuinely new logic, no TT-7.5a analogue does a temporal per-member comparison — 10pts+; **b4** join-time gating — confirmed near-zero: content-access-only decision means `JoinGroupTherapyAction` stays payment-unaware, matching TT-7.5a's own precedent — 0-2pts; **b5** frontend: counsellor-facing toggle (both settings) on GroupTherapy payment-details/create form, `PaymentRequiredBanner.vue` widened to be payable-type-parameterized (not forked) so it also works once `UnifiedTherapy.vue` absorbs GroupTherapy's page — 8pts; **b6** regression closeout: TT-7.5a-style "never regresses individual-Therapy gating" tests (mandatory given how much shared code b2/b3 widen), decision-log entries, feature doc — 3-5pts. Also fixes an inherited TT-7.4d ambiguity at its shared source: a User who creates a GroupTherapy directly becomes a `group_therapy_user` pivot member too (matching `GroupTherapy::getUsers()`'s existing assumption), so `GetGroupTherapyPaymentRosterAction`'s roster and this gate's own membership check agree, rather than two silently-diverging definitions of "member." Sequencing: b0 → b1 → b2 → b3 → b4 → b5 → b6 (b1/b2 NOT parallelizable — b2's guard-widening needs the same "member" definition b1's create-path assumes). Public/non-participant visibility unaffected, same as TT-7.5a. | Story | High | ~34-42 (provisional, floor not ceiling per this epic's own pattern) | New split of TT-7.5 (SCRUM-215 review 2026-09-02); re-scoped via /start-feature (SCRUM-216 review, 2026-09-11) | TT-7.4d (done) |
...
| TT-7.6d | ✅ **Implemented** (SCRUM-228, PR #157, merged). Counsellor-facing frontend: `UpdateCounsellorPayoutDestination.vue` onboarding modal (bank/momo) plus a new private "Payouts" section on the counsellor's own profile page — available balance with itemized gross/fee/net breakdown per pending earning (TT-6.4c pattern), a withdraw control with disabled-reason messaging, payout history, and a retry-after-failure state reusing TT-7.4-retry's amber "try again" styling convention (not its `isRetryStatus` implementation, since `CounsellorPayoutStatusEnum` has no `ABANDONED`-equivalent). New `PayoutController`/`GetCounsellorPayoutOverviewAction`/three `Http\Resources`, both routes self-service only (no `{counsellorId}` param). Playwright-verified golden path, including manually driving the queued payout job to observe the failed/retry state end-to-end. | Story | Medium | 8 | New split of existing TT-7.6 (SCRUM-224 product-owner/project-manager/architect review, 2026-09-02) | TT-7.6a, TT-7.6b, TT-7.6c |
| TT-7.6e | ✅ **Implemented** (SCRUM-229, PR #159, merged). Admin-facing frontend: a dedicated `Admin/Payouts.vue` page (not another `Admin.vue` dispatch-table tab) with a platform-fee/minimum-payout settings form (both keys of TT-7.6b's `platform_settings` mechanism), an admin-on-behalf-of payout-trigger UI (counsellor search reusing `admin.counsellors`, a balance overview, then the existing `payout.trigger` route with `counsellorId`), and a payout audit/history table across every counsellor with failure reasons (new `AdminCounsellorPayoutResource`, distinct from the counsellor-facing one). Completes the TT-7.6 epic. | Story | Low | 5 | New split of existing TT-7.6 (SCRUM-224 product-owner/project-manager/architect review, 2026-09-02) | TT-7.6b, TT-7.6c |
| TT-7.7a (SCRUM-249) | ✅ **Implemented** (PR #192, merged). Refund data model & eligibility: new refund audit-trail schema mirroring `TransactionStatusHistory`'s per-state-change-is-a-row philosophy (architect recommendation, 2026-09-02: a separate `refunds` table — not a `TransactionStatusEnum::refunded` case — since `status` already means "did the charge succeed" for existing consumers, and refunds are naturally 1:many per transaction, not 1:1); the client-request/admin-approval state machine reuses the existing polymorphic `Request` model (new `RequestTypeEnum::refund` case, dispatched via `RespondToRequestAction` to a new `RespondToRefundRequestAction`, mirroring the `organizationCounsellorCompensationChange` precedent) for the ask/approve step ONLY — the actual Paystack call is isolated in its own queued job (TT-7.7d), never inline in the shared request-response dispatcher (architect: keeps a real external money-moving side effect out of a dispatcher already flagged as growing debt, SCRUM-119/120). `EnsureTransactionIsRefundEligibleAction` (SUCCESS-only, full-refund-only, no duplicate pending request; org-paid-transaction eligibility check included, per the user's 2026-09-02 decision to keep org-paid transactions in scope from the start). No FK/read/write relationship to `payment_access_grants`. | Story | Medium | 5 | New split of existing TT-7.7 (SCRUM-223 product-owner/architect review) | TT-7.1, TT-7.2a, TT-7.3b |
| TT-7.7b (SCRUM-250) | ✅ **Implemented** (PR #193, merged). Client refund request: backend (`RequestRefundAction`, client-scoped-to-own-transaction, reason required, one-active-request-per-transaction guard) + admin notification on new request + client-facing request control/reason form/pending-status display on both `TherapyPaymentDetails.vue` (PER_THERAPY) and `UnifiedTherapy.vue` (PER_SESSION), mirroring TT-7.4b's dual-surface pattern. Security-engineer HIGH finding fixed pre-merge: `transactionId`/`refundRequestStatus` scoped to the viewer's own transaction, not the model-wide `latestTransaction`. | Story | Medium | 8 | New split of existing TT-7.7 (SCRUM-223 product-owner review) | TT-7.7a, TT-7.4a, TT-7.4b |
| TT-7.7c (SCRUM-251) | ✅ **Implemented** (PR #194, merged). Admin review queue: admin-only paginated pending-requests list (mirrors TT-6.6d/TT-6.5a's queue-table precedent). Implemented by extending the single `RespondToRefundRequestAction` for both accept/reject (deviation from the ticket's own literal two-action wording, logged) — reject requires a reason and notifies the client immediately; accept creates the `Refund` row. | Story | Medium | 8 | New split of existing TT-7.7 (SCRUM-223 product-owner review) | TT-7.7a, TT-7.7b |
| TT-7.7d (SCRUM-252) | ✅ **Implemented** (PR #195, merged). Real Paystack refund execution: `PaystackClient::refundTransaction()` (new method), fired on TT-7.7c's approve transition via a queued `ProcessRefundJob` (dispatched post-commit, not inline in `RespondToRequestAction`); `refund.processed`/`refund.failed` webhook handling in the existing `ProcessPaystackWebhookJob`, correlating via the ORIGINAL transaction's own reference (Paystack's refund endpoint takes no caller-supplied idempotency reference, confirmed via spike — `refunds.reference` is internal/audit-only). New `RecordRefundStatusAction` choke point (lockForUpdate-based, closing a TOCTOU race found in security review) triggers TT-7.3b-g's `ReconcileOrgFinancedRefundAction` for org-financed transactions, inside the same transaction as the status write. Highest-risk ticket in this split — real money movement; especially thorough reviewer/security-engineer passes. Explicit regression test: never reads/writes `payment_access_grants`. Follow-up filed (SCRUM-255, PR #197, in review): queue-redelivery guard for this job and TT-7.6c's `ProcessCounsellorPayoutJob`. | Story | High | 8 | New split of existing TT-7.7 (SCRUM-223 product-owner review) | TT-7.7c, TT-7.1, TT-7.3b |
| TT-7.7e (SCRUM-253) | ✅ **Implemented** (PR #196, merged). Client outcome notifications (`RefundSucceededNotification`/`RefundFailedNotification`, both explicitly state platform/therapy access is unaffected — product-owner-reviewed, mental-health trust requirement) + counsellor-facing payment-status indicator (TT-7.4c) extended with a new `refundStatus` field so it shows "Refunded" instead of a stale "Paid." Security-engineer finding fixed: `refundStatus` withheld entirely for a GroupTherapy session on `SessionResource` (that resource is multi-payer for groups, unlike individual Therapy) — closes the epic. | Story | Medium | 5 | New split of existing TT-7.7 (SCRUM-223 product-owner review) | TT-7.7d, TT-7.4c |
...
TT-7.5b (GroupTherapy) can now proceed to its own scoping pass — TT-7.4d was scoped 2026-09-10
(SCRUM-256/258-262, 22 points, split a-e) after being unscoped since SCRUM-118. TT-6.1
...
**TT-7.7 epic complete (2026-09-10)**: all five sub-tickets (TT-7.7a-e, SCRUM-249-253) merged —
see their individual rows above. Two follow-ups filed during review, not blocking: SCRUM-255
(queue-redelivery guard for `ProcessRefundJob`/`ProcessCounsellorPayoutJob`, PR #197, in review)
and SCRUM-254 (Markdown-injection hardening for free-text `reason` fields across several
notifications, not yet started). TT-7.4d (group-therapy per-member payment, unscoped since
SCRUM-118) was scoped the same day — see its own TT-7.4d-a…e rows above — after a
product-owner pass surfaced a real, live, already-reachable authorization defect
(`EnsureCanInitiateChargeAction`'s "already paid" guard had no per-member scoping for
GroupTherapy), fixed first and separately as SCRUM-257 (PR #198, in review).
...
**TT-7.6 fully complete (2026-09-03)**: all five sub-tickets (TT-7.6a–e, SCRUM-225 through
SCRUM-229) are merged into `develop`. TT-7.3b (org billing reconciliation) is now unblocked and is
the next active unit of work in this resequenced chain, per the "TT-7.6/TT-7.3b/TT-7.7 status"
note above.

**TT-7.3b fully scoped, rescoped mid-review, and split (2026-09-03, SCRUM-230 product-owner/
project-manager/architect review)**: went through its own `/start-feature` pass and, like every
other TT-7 sub-story reviewed, was found to need a much bigger scope than its original one-line
description. What started as "split money that already came in" turned out to require building
real organization-side payment collection from scratch — investigation confirmed no mechanism
anywhere in the codebase bills or collects money from an org (TT-7.3a only ever built attribution/
tracking), surfacing two live bugs in the process (org-paid counsellors currently earn $0; a
retainer-covered therapy with strict payment gating permanently locks its client out). Twelve
resolved product/technical decisions and 12 sub-tickets (~74 points, TT-7.3b-a through -k, see
rows above) came out of this review — full trail, including several genuine forks the user
resolved directly (e.g. the org pays the actual per-transaction cost rather than the client's
listed price, which cleanly resolves FREE-type compensation as a side effect; retainer billing is
aggregated post-paid invoicing settled via the same saved org payment instrument as pay-per-use;
a suspended org's members are blocked entirely rather than falling back to personal pay), is in
`documentation/decision-log.md`'s 2026-09-03 SCRUM-230 entries. TT-7.3b-a, -b0, -f1, and -k have no
blocking dependencies and can start immediately.
