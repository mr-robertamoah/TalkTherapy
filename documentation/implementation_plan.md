...
| TT-7.3b-d (SCRUM-235) | Payout-split/earnings-generation logic: replaces `GenerateCounsellorEarningsAction`'s org no-op with a real org-specific split (its own gross/fee/net formula, distinct from the personal-pay one, since an org `Transaction.amount` is already fee+share combined) reusing TT-7.6b's `CounsellorEarning` ledger unchanged. **Fully fixes the "$0 for org-paid counsellors" bug.** Buildable/testable against any `organization_id`-set `Transaction` regardless of how it was collected — not blocked on TT-7.3b-a/b/c. Regression invariant: `earning.net_amount + earning.fee_amount == transaction.amount` for every row. | Story | Medium | 8 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-b0, TT-7.6b (done) |
