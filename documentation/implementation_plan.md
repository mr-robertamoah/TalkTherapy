...
| TT-7.6 | Counsellor payout via Paystack Transfers matching `shareEqually`/`sharePercentage`, incl. bank-account/KYC onboarding. Payout is admin/counsellor-triggered separately, never auto-fired on charge success. **Promoted to the active next unit of work (2026-09-02)**: previously "own Epic once scheduled," now scheduled — it's a hard prerequisite for TT-7.3b, which is itself now a hard prerequisite for TT-7.7 (see TT-7.7's note below). Needs its own `/start-feature` pass (Jira ticket + product-owner/project-manager/architect review) before implementation — not yet scoped in detail. | Story | Medium | 13 (provisional, likely to grow on review — same pattern as every other TT-7 sub-story) | New (SCRUM-110) — decision made; promoted from "own Epic once scheduled" to active, per user decision during SCRUM-223 review | TT-7.1 |
