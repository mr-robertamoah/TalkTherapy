...
| TT-7.3b-b (SCRUM-233) | Shared org-charge primitive: computes the actual cost (platform fee via `SettingsService` + counsellor's compensation-driven share via TT-7.3b-b0) and charges it to the org's saved payment instrument (TT-7.3b-a) via the new `PaystackClient::chargeAuthorization()`, producing a `Transaction` row with `organization_id` set. The one mechanism both TT-7.3b-c (pay-per-use) and TT-7.3b-e (retainer settlement) call into — never a separate implementation for each. | Story | Medium | 6 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-a, TT-7.3b-b0 |
