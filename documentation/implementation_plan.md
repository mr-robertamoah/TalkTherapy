...
| TT-7.3b-c (SCRUM-234) | Pay-per-use org collection wiring: rewires `EnsureOrganizationCanPayForModelAction`/`InitiatePaystackChargeAction` so a pay-per-use org member's session charges the org's instrument (via TT-7.3b-b) at actual-cost, instead of charging the member's own card the full listed price as today. | Story | Medium | 5 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-7.3b-b |
