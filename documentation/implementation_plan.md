...
| TT-7.3b-a (SCRUM-231) | Org payment-instrument backend: a Paystack capability spike (mirrors TT-7.6a's own spike) confirming `chargeAuthorization` (charging a previously-returned `authorization_code` on demand) supports this recurring/on-demand use case, then a new org-payment-instrument table + org-admin backend flow to register one, masked-display-only storage (mirrors TT-7.6a's `recipient_code`-only convention — no raw card data persisted). | Story | Medium | 8 | New split of TT-7.3b (SCRUM-230 review, 2026-09-03) | TT-6.1 (org verification, done) |
