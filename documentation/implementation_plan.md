...
| TT-7.4b | Client Pay button + redirect + return-flow status UI, individual therapy & session only (group therapy explicitly excluded — see TT-7.4d). New `usePayment(therapy, therapyType)` composable (matching `useTherapyState`/`useAlert`/`useErrorHandler` conventions) owns initiate/redirect (`window.location.href`, never Inertia `router.visit`)/status/dismiss logic, reusing `useTherapyState`'s `computedIsParticipant`/`computedIsCounsellor` for the pay-gating check; consumed by both `TherapyPaymentDetails.vue` and `UnifiedTherapy.vue`'s session-actions modal rather than logic living in one and being reused by the other. Dismissible one-time status banner (not persistent), all `TransactionException` messages surfaced distinctly (incl. 429 throttle), "abandoned" treated as recoverable not terminal (backend doesn't finalize on it either). Also gives `CreateSessionFormModal.vue`'s existing payment copy a concrete reachable Pay action. Mandatory Playwright golden path. | Story | High | 8 | New split of existing TT-7.4 (SCRUM-118 product-owner/architect review); filed as SCRUM-157 | TT-7.4a |
| TT-7.4c | Counsellor read-only payment-status indicator on the same two surfaces as TT-7.4b — status text only, Pay control never rendered for the counsellor (backend already 403s via `EnsureCanPayForModelAction`; this is UI-layer control-hiding, reviewed separately as a distinct authorization-adjacent concern). | Story | Medium | 3 | New split of existing TT-7.4 (SCRUM-118 product-owner/architect review); filed as SCRUM-158 | TT-7.4a |
...
| TT-7.4-retry | Retry-on-failure UI (TT-7.4's original second clause) — not covered by any of SCRUM-118's FRs; carved out as its own small follow-up rather than silently dropped in the a/b/c/d renumbering. | Task | Medium | TBD | Split out of original TT-7.4 (SCRUM-118 project-manager review) | TT-7.4b |
...
**Sprint:** 5 (TT-7.1 alongside TT-6). TT-7.4a/b/c/5/7 are fast-follows immediately after TT-7.1
