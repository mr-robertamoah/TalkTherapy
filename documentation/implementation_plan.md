...
| TT-3.2 | Group therapy video rooms (multi-party) — NOT yet scoped in depth; real shape depends on TT-3.1's own implementation landing first (both chosen providers support multi-party natively, so this should mostly be "relax the 2-participant business rule," not a re-architecture — confirm once TT-3.1 ships). | Story | High | 13 | New | TT-3.1 |
