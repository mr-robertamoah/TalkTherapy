...
const scrollItems = [
  { id: "therapy_background_story", name: "about" },
...
watchEffect(() => {
  ...
  if (props.recentSessions?.length) recentSessions.value = [...props.recentSessions];
});
watchEffect(() => {
  if (props.recentTopics?.data?.length) recentTopics.value = [...props.recentTopics.data];

  if (props.recentTopics?.length) recentTopics.value = [...props.recentTopics];
});
watchEffect(() => {
...
function listenToTherapy(currentTherapy) {
  Echo.join(`groupTherapies.${currentTherapy.id}`)
    .here((users) => {
    ...
    .listen(`.session.updated`, (data) => {
      if (activeSession.value?.id == data.session.id) activeSession.value = data.session;
    })
    .listen(`.session.topic.set`, (data) => {
      if (activeSession.value) activeSession.value.currentTopic = data.topic;
    })
    .listen(`.session.topic.unset`, (data) => {
      if (activeSession.value) activeSession.value.currentTopic = null;
    })
    .listen(`.discussion.started`, (data) => {
      activeDiscussion.value = data.discussion;
      startTimer();
    })
    .listen(`.discussion.updated`, async (data) => {
...
onBeforeUnmount(() => {
  ...
  Echo.leave(`groupTherapies.${currentTherapy.id}`);
});
...
function isNotParticipant() {
  ...
  return !counsellor && userId !== user.id;
}
...
function clickedShowAll() {
  ...
  if (computedTherapy.value?.activeDiscussion && !activeDiscussion.value) {
    activeDiscussion.value = computedTherapy.value.activeDiscussion;
    startTimer();
  }
...
async function clickedResponse(response) {
  ...
  await axios
    ...
    .then((res) => {
      ...
      if (res.data.request?.status == "ACCEPTED") counsellors.value.push(res.data.request.to);
...
function deleteSessionOrTopic(item) {
  if (item.isSession) {
    recentSessions.value.splice(
      recentSessions.value.findIndex((session) => session.id == item.id),
      1
    );
    return;
  }

  recentTopics.value.splice(
    recentTopics.value.findIndex((session) => session.id == item.id),
    1
  );
}
...
function updateSessionOrTopic(item) {
  if (item.isSession) {
    if (activeSession.value?.id == item.id) activeSession.value = item;
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    >
      <div
        v-if="activeDiscussion || computedTherapy.activeDiscussion"
        class="w-[80%] md:w-[70%] lg:w-[60%] mb-2 mx-auto"
      >
        <div
          @click="clickedShowAll"
    ...
    <div class="pt-0 pb-12">
      ...
      <div
        class="bg-gray-100 z-[2] top-0 max-w-7xl mx-auto sm:px-6 lg:px-8 my-4 p-2 flex justify-start items-center overflow-x-auto overflow-hidden"
      >
        <div
          v-for="item in scrollItems.filter((item) =>
            computedTherapy.paymentType == 'PAID'
              ? item
      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div
          class="overflow-hidden shadow-sm sm:rounded-lg my-8 flex flex-col space-y-4 md:space-y-0 md:grid md:grid-cols-2 md:space-x-4"
        >
          <div
            ref="mainDiv"
            class="flex space-x-4 items-start justify-start w-full overflow-hidden overflow-x-auto pb-2"
          >
            ...
            <div class="w-full shrink-0" id="therapy_participants">
                  ...
                  <template v-else>
                    ...
                    >
                      <PrimaryButton
                        ...
                        @click="() => clickedResponse('accepted')"
                        class="shrink-0"
                        >accept</PrimaryButton
                      >
                      <DangerButton
                        :disabled="request.responding"
                        @click="() => clickedResponse('rejected')"
                        class="shrink-0"
                        >reject</DangerButton
                      >
                    </div>
              ...
              <div
                v-if="computedIsUser && !computedTherapy.counsellors.length"
              ...
              >
                ...
                >
                  ...
                  <div v-else class="h-10 flex justify-center items-center w-full">
                    no links for counsellors as at now.
                  </div>
                </div>
            ...
            <div class="bg-white p-6 shrink-0 w-full" id="therapy_details">
              ...
              <div class="my-4">
                <NameAndValue
                  :name="'Payment Type'"
                  :value="computedTherapy.paymentType"
                />
                <NameAndValue
                  :name="'Session Type'"
                ...
                <div class="flex p-2 items-center justify-start">
                  ...
                  <div v-else class="text-gray-600 text-sm text-center my-2">
                    there are no therapy cases added.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div
              class="bg-white p-6 shrink-0 w-full"
              v-if="computedTherapy.paymentType == 'PAID'"
              id="therapy_payment_details"
            >
              ...
              <div class="my-4">
                <div class="flex justify-start items-center mb-4">
                  ...
                  <div class="p-2 border-stone-600 text-start min-w-[120px]">
                    {{
                      computedTherapy.paymentData.currency +
                      " " +
                      computedTherapy.paymentData.amount
                    }}
                  </div>
                </div>
                <div
                  class="flex justify-start items-center mb-4"
                  v-if="computedTherapy.allowInPerson"
                >
                  <div
                    class="text-sm text-gray-600 p-2 border-b-2 border-stone-600 mr-2 min-w-[130px] text-end"
                  >
                  ...
                  <div class="p-2 border-stone-600 text-start min-w-[120px]">
                    {{
                      computedTherapy.paymentData.currency +
                        " " +
                        computedTherapy.paymentData.inPersonAmount ??
                      computedTherapy.paymentData.amount
                    }}
                  </div>
                </div>
            ...
            <div class="bg-white p-6 shrink-0 w-full" id="therapy_other_details">
              ...
              <div class="text-gray-600 text-sm mt-2">
                green means true and otherwise false
              </div>
              <div class="my-4">
                <BooleanAttribute
                  :text="'anonymous'"
                ...
                />
                <BooleanAttribute
                  :text="'allows any user to join without request'"
                  :condition="computedTherapy.allowAnyone"
                  class="my-2"
                />
              </div>
            </div>
            ...
            <template v-if="computedTherapy.status !== 'ENDED'">
              ...
              <PrimaryButton
                ...
                v-if="
                  ...
                  computedTherapy.maxSessions > computedTherapy.sessionsHeld
                "
                >create session</PrimaryButton
              >
              <PrimaryButton
                @click="clickedCreateDiscussion"
                class="shrink-0"
              ...
              <template v-if="!computedIsInSession">
                <PrimaryButton
                  @click="clickedEndTherapy"
                  v-if="
                    computedIsParticipant &&
                    computedTherapy.status !== 'ENDED' &&
  ...
  <MiniModal
    :show="
      modalData.show && ['request assistance', 'delete', 'end'].includes(modalData.type)
    "
    @close="closeModal"
  >
    <div class="select-none">
      ...
      <template v-if="modalData.type == 'end'">
        <div class="text-gray-600 text-center font-bold tracking-wide">End Therapy</div>
  ...
  <CreateDiscussionFormModal
    ...
    @on-success="
      (data) => {
        if (data) discussions.data = [data, ...discussions.data];
      }
    "
  />
